...
public class EmployeeOperationsController {
	...
	public  String     saveEmployee(@ModelAttribute("emp") Employee emp,
		...
		System.out.println("EmployeeOperati{onsController.saveEmployee()");
		//enable  server side form validations only when client side form validations not done
	if(emp.getVflag().equalsIgnoreCase("disabled")) {	
		//enable server side form validations
	...
	*/
	...
	public    String    updateEmployee(@ModelAttribute("emp") Employee emp,
		...
		if(validator.supports(emp.getClass())) {
			 validator.validate(emp, errors);
			 if(errors.hasErrors()) {
				 return  "edit_employee_form";  //LVN of the form page
			 }
		  }
		}
